...
- Adds `Deprecation` header (callers should migrate to multi-resume flows)

---

## Modified: GET /api/resume

**Changes from current behavior**:
- Returns the **default** resume (via `users.default_resume_id`) instead of most recent
