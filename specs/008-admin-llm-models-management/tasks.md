...
- [x] T032 [US3] Update parse/generate endpoints to use new runtime options in `packages/nuxt-layer-api/server/api/resumes/index.post.ts`, `packages/nuxt-layer-api/server/api/resumes/index.post.ts`, and `packages/nuxt-layer-api/server/api/vacancies/[id]/generate.post.ts` (package: `@int/api`)
